...
```{r}
tidy.ELA <- tidy.ELA %>%
  filter(!is.na('Student ID'))  ## if use this function to delete NA, would have some problem here!!

tidy.ELA %>%
  filter('Student ID' >0) ## It also doesn't work to delete the NA in student ID.
```
...
```{r}
# Solution 1. This one doesn't work.
tidy.ELA2 %>%
  group_by(Room) %>%
  count(is.na(`1st Quarter`) == FALSE)  ## Why does it also show FALSE count?
```
...
median_plot  ## I don't know why the plot is blank :(
```
...
NYC
NYC2 <- filter(NYC, origin == "JFK", dest == "ATL" )
NYC2 <- select(NYC2, arr_delay, carrier, origin, dest)
NYC2
## These two seperate functions do work. However, if I put them together, it doesn't work. As the following chunck shows.

```

```{r}
(NYC2 <- NYC %>%
     filter(NYC, origin == "JFK", dest == "ATL" ) %>%
     select(arr_delay, carrier, origin, dest)) ## This one doesn't work.
